    ...
    if (response.data.files && response.data.files.length > 0) {
      ...
      while (true) {
        ...
        if (
          ...
          checkResponse.data.files.length === 0
        ) {
          break;
        }
...
export async function POST(req: Request) {
  try {
    ...
    const replaceExisting =
      (formData.get("replaceExisting") as string) === "true";

    console.log(
      `Upload mode: ${uploadMode}, target folder: ${folderId}, replace: ${replaceExisting}`,
    );
    ...
    if (uploadMode === "folder" && folderStructures.size > 0) {
      ...
      for (const folderPath of sortedPaths) {
        ...
        const newFolderId = await createUniqueFolder(
          folderInfo.name,
          parentId,
          drive,
        );
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        if (uploadMode === "folder" && relativePath) {
          ...
          if (dirPath && dirPath !== ".") {
            ...
            if (!targetFolderId) {
              ...
              console.log(
                `Folder path not found: ${dirPath}, using default folder`,
              );
        ...
        if (fileExists.exists && replaceExisting) {
          ...
          console.log(
            `Replacing existing file: ${fileName} (${fileExists.fileId})`,
          );
    ...
    const duplicateCount = results.filter(
      (r) => r.action === "created_duplicate",
    ).length;

    console.log(
      `=== UPLOAD API COMPLETE: ${successCount}/${files.length} files processed ===`,
    );
    console.log(
      `=== ${replacedCount} replaced, ${newCount} new, ${duplicateCount} duplicates ===`,
    );
